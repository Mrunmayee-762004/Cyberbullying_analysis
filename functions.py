...
def custom_input_prediction(text):
    ...
    for i in interpretations.keys():
        if i == prediction:
            ...
            data.to_csv('C:/Users/mrunm/Downloads/ML_CP_TY/predictions.csv', mode='a', header=not data.empty, index=False)
            # data.to_csv('predictions.csv', mode='a', header=not data.empty, index=False)
            return result
